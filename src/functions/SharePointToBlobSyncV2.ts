...
export async function SharePointToBlobSyncV2(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    context.log(`SharePoint to Blob sync function (Graph API) started at: ${new Date().toISOString()}`);
    ...
    try {
        ...
        await containerClient.createIfNotExists({
            metadata: {
                purpose: 'sharepoint-extracted-content-graph',
                createdBy: 'azure-function-graph'
            }
        });
        ...
        const files: SharePointFile[] = filesResponse.data.value
            ...
            .map((item: any) => {
                ...
                return {
                    ...
                    size: driveItem.size,
                    fileExtension: driveItem.name.split('.').pop()?.toLowerCase() || ''
                };
            });
...
async function processFileFromGraph(
    ...
    try {
        ...
        const blobName = `${file.name.replace(/\.[^/.]+$/, '')}_${new Date(file.lastModified).getTime()}.json`;
        ...
        const extractedText = await extractTextContent(fileBuffer, file.fileExtension, context);
...
async function downloadFileFromGraph(downloadUrl: string, context: InvocationContext): Promise<Buffer> {
    try {
        ...
        context.log(`Downloaded file: ${buffer.length} bytes`);
        return buffer;
    } catch (error) {
        context.log(`Error downloading file: ${error.message}`);
...
app.http('SharePointToBlobSyncV2', {
    methods: ['GET', 'POST'],
    authLevel: 'function',
    handler: SharePointToBlobSyncV2
});
